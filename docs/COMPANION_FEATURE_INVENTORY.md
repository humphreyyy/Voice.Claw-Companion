...
1. The menu bar extra uses a system symbol rather than a custom branded icon.
2. The public HTTPS bridge setup is intentionally external. The Companion can store and pair the URL, but it does not create a Cloudflare Tunnel or public DNS record.
3. The updater checks for a DMG and downloads it, but it does not perform a Sparkle-style in-place update.
4. The Companion and runtime have overlapping descriptions of routes and tools; these should eventually be generated from a shared capability registry.
5. The local web client still exists for debug/runtime parity. Confirm whether it should remain packaged in user-facing releases.
6. Keep release automation strict: no unnotarized DMG, no malformed release notes, and no accidental contributor attribution.
